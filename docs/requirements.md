...
APIGuardian is a API governance and security for backend teams, platform security, API product owners. The product must support the complete path from setup to reviewed output, with enough traceability and operational control for real use by a small team.

## 2. Goals

- Deliver one reliable end-to-end workflow for API governance and security.
...
- OpenAPI import, contract testing, auth-policy checks, breaking-change detection, audit reports
...
- The product must provide a guided intake form for API governance and security.
...
- The product should support at least one integration handoff in MVP: OpenAPI, Postman, CI pipelines, API gateways.
...
Primary risk areas: breaking production clients, leaked secrets, incomplete endpoint inventory, policy bypass.
